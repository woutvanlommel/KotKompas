<div class="flex flex-col h-full">
    ...
    @elseif($conversation)
        ...
        <div class="border-t border-[#0f17201f] shrink-0">
            ...
            @else
            @if($inReplyWindow)
                <div class="max-w-3xl mx-auto w-full flex items-start gap-3 px-4 sm:px-6 pt-4 text-[#586573]">
                    <x-heroicon-o-clock class="w-5 h-5 shrink-0 mt-0.5 text-[#9aa6b4]" />
                    <p class="text-sm leading-relaxed">
                        Je verhuurder heeft je een tijdelijk antwoordvenster gegeven. Je kunt reageren tot
                        {{ $conversation->tenant_unlocked_until->setTimezone(config('app.timezone'))->format('d/m/Y H:i') }}.
                    </p>
                </div>
            @endif
