        ...
        <div class="flex flex-col gap-8 rounded-2xl border border-hairline bg-canvas-deep p-6 md:flex-row md:items-center md:gap-12 md:p-8">
            ...
            <div class="shrink-0 md:w-56">
                <p class="flex items-baseline gap-1.5">
                    <span class="text-[clamp(2.5rem,4vw,3.5rem)] font-medium leading-none tracking-[-0.04em] text-ink">{{ \App\Support\Score::format($room->score) }}</span>
                    ...
                    @foreach ($criteria as $label => $value)
                        <div>
                            <div class="mb-1.5 flex items-baseline justify-between gap-4">
                                <dt class="text-[0.7rem] font-medium uppercase tracking-widest text-ink/70">{{ $label }}</dt>
                                <dd class="text-sm font-semibold text-ink">{{ \App\Support\Score::format($value) }}</dd>
                            </div>
