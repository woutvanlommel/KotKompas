...
function initReveals(): void {
    ...
    if (reduceMotion || !firstVisit) {
        gsap.set("[data-reveal], [data-split], [data-converge]", {
            opacity: 1,
            x: 0,
            y: 0,
        });
        gsap.utils
            ...
            .forEach((g) => gsap.set(g.children, { opacity: 1, x: 0, y: 0 }));
    ...
    document.querySelectorAll<HTMLElement>("[data-split]").forEach((el) => {
        ...
        split.lines.forEach((line) => {
            ...
            wrap.style.cssText =
                "display:block;overflow:hidden;padding-block:0.12em;margin-block:-0.12em;";
    ...
    if (stack) {
        ...
        if (items.length > 1 && window.innerWidth >= 768) {
            ...
            items.forEach((item, i) => {
                ...
                const inner =
                    item.querySelector<HTMLElement>(".kk-stack-inner");
                if (!inner) return;
                const targetScale = Math.max(
                    0.86,
                    1 - (items.length - 1 - i) * 0.04,
                );
                gsap.fromTo(
                    inner,
                    { scale: 1, rotate: 0 },
...
const adaptiveNav = document.querySelector<HTMLElement>(
    ".kk-nav[data-adaptive]",
);
if (adaptiveNav) {
    const onScroll = (): void => {
        adaptiveNav.classList.toggle(
            "is-solid",
...
if (finePointer && !reduceMotion) {
    ...
    if (cards.length) {
        ...
        label.innerHTML =
            '<span>Bekijk</span><svg viewBox="0 0 16 16" fill="none"><path d="M4 12L12 4M12 4H6M12 4V10" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
if (faqSearch) {
    ...
    const cats = Array.from(
        document.querySelectorAll<HTMLElement>("[data-faq-cat]"),
    );
    ...
    faqSearch.addEventListener("input", () => {
        ...
        cats.forEach((cat) => {
            cat.hidden = !Array.from(
                cat.querySelectorAll<HTMLElement>("[data-faq-item]"),
            ).some((i) => !i.hidden);
        });
